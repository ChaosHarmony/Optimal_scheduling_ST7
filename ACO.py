...
def ACO_basic_ants(graph: nx.DiGraph, num_machines:int = 2, num_ants:int = 10, alpha: float = 1.0, beta:float = 5.0, evaporation_rate: float = 0.5, q:float = 1.0, num_iterations:int = 100):
    ...
    for it in range(num_iterations):
        ...
        for ant in range(num_ants):
            ...
            current_node = np.random.choice(available_nodes, p=probabilites_construction(alpha, beta, eta, pheromone_matrix, jobs_to_index_mapping, None, available_nodes))
            ...
            while len(visited_jobs) < len(graph):
                available_nodes += get_next_avaiable_jobs(graph=graph, visited_jobs=visited_jobs, completed_job=current_node)
                next_node = np.random.choice(
                    available_nodes,
                    p = probabilites_construction(alpha, beta, eta, pheromone_matrix, jobs_to_index_mapping, current_node=current_node, available_nodes=available_nodes)
                    )
            ...
            for i in range(len(ant_path) - 1):
                pheromone_matrix[jobs_to_index_mapping[ant_path[i]], jobs_to_index_mapping[ant_path[i+1]]] += q/makespan(machines)
